...
# SQLite
# NC_*/NCP_DIR are the legacy entry points used by this repo's test scripts;
# SQLITE_* derives from them so an exported NC_DIR / NC_CONTAINER still wins.
NCP_DIR="${NCP_DIR:-$REPO_ROOT/NCP}"
NC_DIR="${NC_DIR:-$REPO_ROOT/national-connector}"
NC_CONTAINER="${NC_CONTAINER:-national-connector}"

SQLITE_CONTAINER="${SQLITE_CONTAINER:-$NC_CONTAINER}"
export SQLITE_DATABASES=(undo-db.sqlite local-lms-db.sqlite job-queue.sqlite)
SQLITE_DATA_DIR="${SQLITE_DATA_DIR:-$NC_DIR/data}"
...
SQLITE_COMPOSE_FILE="${SQLITE_COMPOSE_FILE:-$NC_DIR/docker-compose.yml}"
