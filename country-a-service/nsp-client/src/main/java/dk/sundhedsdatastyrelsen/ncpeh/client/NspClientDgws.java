...
public class NspClientDgws {
    ...
    public static Element getSignedOrganizationCard(URI uri, CertificateAndKey certificate, Instant now) throws AuthenticationException {
        try {
        ...
        } catch (AuthenticationException | NspClientException | IOException | NoSuchAlgorithmException |
                 CertificateEncodingException | TransformerException | XPathExpressionException e) {
            throw new AuthenticationException("Error getting signed ID from DGWS STS.", e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new AuthenticationException("Error getting signed ID from DGWS STS, thread interrupted.", e);
        }
    }
